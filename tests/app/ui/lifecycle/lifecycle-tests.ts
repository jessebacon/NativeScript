...
export function test_css_sets_properties() {
    ...
    const expectedChangesAfterReaddedToPage = [1, 3, 3, 3];
    ...
    for (let i = 0; i < buttons.length; i++) {
        ...
        TKUnit.assertEqual(buttons[i].colorPropertyChangeCount, expectedChangesAfterReaddedToPage[i], `Expected ${buttons[i].id} change notifications for css properties to not occur when added to page.`);
    }
}
